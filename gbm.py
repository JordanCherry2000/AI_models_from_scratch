...
def gbm_create_tree(X_train, y_train, num_iters, learning_rate):
    F_train = np.full_like(y_train, y_train.mean(), dtype = np.float64)
    trees = []
    for i in range(num_iters):
        residuals =  y_train - F_train
        indices = np.arange(len(y_train))
        tree = build_tree(X_train, residuals, indices, 0)
        predictions = predict_tree(X_train, tree)


        F_train += predictions * learning_rate
        trees.append(tree)
    return trees


def gbm_pred_tree(X_test, trees, learning_rate):
    F_test = np.full_like(y_test, y_test.mean(), dtype=np.float64)
    for i in range(len(trees)):
        residuals = F_test - y_test
        indices = np.arange(len(y_train))
        predictions = predict_tree(X_test, trees[i])
        F_test += predictions * learning_rate
    return F_test

trees = gbm_create_tree(X_train, y_train, 100, 0.05) 
F_test = gbm_pred_tree(X_test, trees, 0.05) 
mse = np.mean((F_test - y_test) ** 2) 
print(mse)
